...
function ThemeToggle() {
  const [theme, setTheme] = useState(() => {
    ...
    const stored = localStorage.getItem('theme') || localStorage.getItem('jvdt:theme');
  ...
  // Also apply theme on initial mount
  useEffect(() => {
    if (theme === 'dark') {
      document.documentElement.classList.add('dark');
    } else {
  ...
  return (
    <button
      ...
      className="px-3 py-1 rounded-md border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
    >
      {theme === 'dark' ? '🌙' : '☀️'}
    </button>
  );
      ...
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <nav className="flex gap-2">
            <Link to="/" className={`px-3 py-1 rounded-lg text-sm ${loc.pathname === '/' ? 'bg-slate-900 text-white' : 'bg-white dark:bg-gray-800 text-slate-800 dark:text-gray-200'}`}>
              Home
            </Link>
