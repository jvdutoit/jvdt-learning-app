...
export default function TestList() {
  ...
  useEffect(() => {
    loadTestHistory();
    loadTestDefinitions();
  }, [language]); // Changed from [t] to [language] to prevent infinite loop

  const loadTestDefinitions = async () => {
    setLoading(true);
    try {
      // Load all test definitions in parallel for better performance
      const [englishModule, jvdt2Module, jvdt4Module, jvdt7Module] = await Promise.all([
        import('../../data/tests/english-fluency.json'),
        import('../../data/tests/jvdt-2.json'),
        import('../../data/tests/jvdt-4.json'),
        import('../../data/tests/jvdt-7.json')
      ]);
      ...
      const testDefinitions = [
        ...
        // JVDT-2 (optimized - only translate basic metadata, not all questions)
        {
          ...jvdt2Module.default,
          title: t('jvdt2.title', jvdt2Module.default.title),
          description: t('jvdt2.description', jvdt2Module.default.description),
          estimatedTime: t('jvdt2.estimatedTime', jvdt2Module.default.estimatedTime),
          targetAudience: t('jvdt2.ageGroup', jvdt2Module.default.targetAudience),
          icon: '🎨',
          categories: [
          ...
          ageGroup: t('jvdt2.ageGroup', jvdt2Module.default.targetAudience)
        },
        // JVDT-4
        {
          ...jvdt4Module.default,
